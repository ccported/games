function alertCustom(title, message, accept, callback) {
  try {
    var alertButtonDelay = new Date().getTime();
    var alertContainer = document.createElement("div");
    alertContainer.className = "alert_confirm_input_custom";
    ...
    alertForm.style.overflow = "hidden";
    alertForm.style.boxShadow = "0 2px 2px 1px rgba(0,0,0,0.4)";
    ...
    alertButton.addEventListener("keydown", function (event) {
      event.preventDefault();
    });
    alertButton.addEventListener("keyup", function (event) {
      if (
        event.keyCode == 13 &&
        new Date().getTime() > alertButtonDelay + 200
      ) {
        callback();
